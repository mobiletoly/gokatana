...
func setupTestCache(t *testing.T) (*KVTCache, context.Context) {
	...
	dbFile := ":memory:"
	cache, err := NewKVTCache(ctx, dbFile, nil)
...
func TestKVTCache_JSONValueType(t *testing.T) {
	cache, ctx := setupTestCache(t)

	// Register a collection for JSON objects.
	collection := katcache.Collection{
		Name:      "test_json_cache",
		Ttl:       10 * time.Second,
	...
	// Test Set and Get operations for JSON objects.
	key := katcache.CollectionKey{Name: "test_json_cache", Key: "key1"}
	err := cache.Set(ctx, key, data)
	require.NoError(t, err)

	var fetchedData TestData
	found, err := cache.Get(ctx, key, &fetchedData)
	require.NoError(t, err)
	assert.True(t, found, "Expected to find the key in the cache")
	assert.Equal(t, data, fetchedData, "Expected fetched data to match the set data")
}

func TestKVTCache_ExpirationWithRunner(t *testing.T) {
	cache, ctx := setupTestCache(t)

	// Register a collection with a short TTL.
	collection := katcache.Collection{
		Name:      "test_cache_with_runner",
	...
	cancel := cache.RunInBackgroundWithCancellation(ctx)
	t.Cleanup(cancel)

	// Test Set and Expiration with Runner.
	key := katcache.CollectionKey{Name: "test_cache_with_runner", Key: "key1"}
	value := "value1"
	err := cache.Set(ctx, key, value)
	require.NoError(t, err)
	...
	found, err = cache.Get(ctx, key, &fetchedValue)
	require.NoError(t, err)
	assert.False(t, found, "Expected the key to have expired and been removed by the runner")
}

func TestKVTCache_GetOrSet_JSON(t *testing.T) {
	cache, ctx := setupTestCache(t)
